...
PTDS_BASE DS_BASE;
PTBTS_BUFFER BTS_BUFFER[MAX_BUFFERS];
int current_buffer=0;
UINT64 ready_buffer=-1;
...
void setup_bts()
{
	...
	{
		BTS_BUFFER[i]=(PTBTS_BUFFER)ExAllocatePool(NonPagedPool,sizeof(TBTS_BUFFER)*SIZE_BTS_BUFFER);
	}

	ext->DS_BASE=DS_BASE;
	ext->BTS_BUFFER=BTS_BUFFER[current_buffer];

	FILL_DS_WITH_BUFFER(DS_BASE,BTS_BUFFER[current_buffer]);
